...
| `utils/RELEASE.md` | How to publish. Tagging is the version bump. **Never publish.** |
...
* **The version is the git tag.** `pyproject.toml` declares `dynamic = ["version"]`
  ...
  releases you get `0.3.1.dev13+g1782fb9`. `utils/RELEASE.md` is the procedure, and
  it is the maintainer's: never tag and never publish unless asked outright.
